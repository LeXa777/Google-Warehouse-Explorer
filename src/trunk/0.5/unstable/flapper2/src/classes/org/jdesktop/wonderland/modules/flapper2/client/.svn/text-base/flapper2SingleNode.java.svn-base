 ...
 * @author morrisford
 */
public class flapper2SingleNode 
    {
    ...
    public flapper2SingleNode(flapper2Renderer Renderer, String nodeName)
        {
        renderer = Renderer;
...
// Add each key frame info as an animation and then play the whole
        for(int j = 0; j < Tkft.length - 1; j++)
            ...
            t.addPropertyToInterpolate("Quat", keyFrames[j], keyFrames[j + 1], new flapper2QuaternionInterpolator());
            ...
            t.addPropertyToInterpolate("Trans", keyTrans[j], keyTrans[j + 1], new flapper2VectorInterpolator());
...
//            flapper2QuaternionAnimation theQuatProc = new flapper2QuaternionAnimation(renderer.getTheEntity(), theSpatial, keyFrames[0], keyFrames[5], keyTrans[0], keyTrans[5]);
...
/*
        ...
        for(int j = 0; j < Tkft.length - 1; j++)
            ...
            System.out.println("kft - " + Tkft[j]);
                ...
                flapper2QuaternionAnimation theQuatProc = new flapper2QuaternionAnimation(renderer.getTheEntity(), theSpatial, keyFrames[j], keyFrames[j + 1], keyTrans[j], keyTrans[j + 1]);
