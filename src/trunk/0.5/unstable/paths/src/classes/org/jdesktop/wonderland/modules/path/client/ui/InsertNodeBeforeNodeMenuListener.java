...
/**
 * This class is used to listen for events to insert a PathNode before a given PathNode in a NodePath.
 *
 * @author Carl Jokl
 */
public class InsertNodeBeforeNodeMenuListener implements ContextMenuActionListener {

    /**
     * The display text for this command.
     */
    public static final String DISPLAY_TEXT = "Insert Node Before";

    /**
     * This method is called when the context menu item to insert a PathNode into a NodePath is fired.
     *
     * @param event The ContextMenuItemEvent containing information about the PathNode before which to insert a new PathNode.
     */
    @Override
        ...
        if (menuItem instanceof EntityContextMenuItem) {
            ...
            if (entity != null && entity.hasComponent(PathNodeComponent.class)) {
                ...
                if (node != null && cell instanceof PathCell) {
                    ...
                    HeightChoiceDialogRunner.showHeightSelectionUsingAvatar(new InsertNodePositionable(path, true, node.getSequenceIndex()), node.getPosition(), cell.getLocalToWorldTransform());
                }
            }
        }
    }
