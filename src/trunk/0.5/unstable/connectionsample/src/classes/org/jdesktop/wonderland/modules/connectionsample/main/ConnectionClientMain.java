 ...
 * ColorChangeConnection, and proceeds to create a cell and then change its
 * color every second.
 *
...
public class ConnectionClientMain {
     ...
     * the given server
     */
    public ConnectionClientMain(String serverURL, String username, 
                                File passwordFile)
        throws ConnectionFailureException, InterruptedException
    {
        ...
        final ColorChangeConnection ccc = new ColorChangeConnection();
        session.connect(ccc);

        // request that a new cell get created
        System.out.println("Requesting cell creation");
        final CellID cellID = ccc.createCell();
        System.out.println("Created cell " + cellID);

        // schedule a task to change colors every second
        timer.schedule(new TimerTask() {
            ...
            public void run() {
                ...
                // use the custom connection to update the color
                ccc.setColor(cellID, color);
            }
        }, 1000, 1000);
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            new ConnectionClientMain(serverURL, username, passwordFile);
        } catch (Exception ex) {
