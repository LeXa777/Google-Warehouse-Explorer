...
public class RoundEllie4ANode
    ...
    public void initializeChildNode()
        ...
        for(int j = startFrame; j < endFrame; j++)
            {
            t = new Timeline(this);
            t.addPropertyToInterpolate("NodeQuat", nodeKeyFrames[j], nodeKeyFrames[j + 1], new RoundEllie4QuaternionInterpolator());
            t.addPropertyToInterpolate("NodeTrans", nodeKeyTrans[j], nodeKeyTrans[j + 1], new RoundEllie4VectorInterpolator());
...
// Add each key frame info as an animation and then play the whole
        ...
        for(int j = startFrame; j < endFrame; j++)
            ...
            t.addPropertyToInterpolate("NodeQuat", quat, quatNext, new RoundEllie4QuaternionInterpolator());
            ...
            if(v3fFinal.x == 0.0f && v3fFinal.y == 0.0 && v3fFinal.z == 0.0)
                ...
                t.addPropertyToInterpolate("NodeTrans", tempVec, tempVecPlus, new RoundEllie4VectorInterpolator());
                }
            else
                {
                Vector3f tempVec = new Vector3f(
    ...
    public void setNodeTrans(Vector3f newTrans) throws IOException
        {
        nodeSpatial.setLocalTranslation(newTrans);
        ClientContextJME.getWorldManager().addToUpdateList(nodeSpatial);
        if(takeAvatar)
            {
            ClientContextJME.getClientMain().gotoLocation(null, newTrans, rotFinal);
            System.out.println("Avatar vector = " + newTrans);
            }
        }
