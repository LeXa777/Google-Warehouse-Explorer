 ...
 * @author morrisford
 */
public class RoundEllie4MultipleNodes
    {
    ...
    private Spatial node6theSpatial;
    private Spatial node7theSpatial;
    private float Tkft[];
    ...
    private Quaternion[] node6keyFrames;
    private Vector3f[] node6keyTrans;
    private Quaternion[] node7keyFrames;
    private Vector3f[] node7keyTrans;
    ...
    private TimelineScenario node6result;
    private TimelineScenario node7result;
    ...
    private String node6Name = null;
    private String node7Name = null;

    public RoundEllie4MultipleNodes(RoundEllie4Renderer Renderer, String node1Name)
        {
        nodeCount = 1;
    ...
    public RoundEllie4MultipleNodes(RoundEllie4Renderer Renderer, String node1Name, String node2Name)
        {
        nodeCount = 2;
        this.node1Name = node1Name;
    ...
    public RoundEllie4MultipleNodes(RoundEllie4Renderer Renderer, String node1Name, String node2Name, String node3Name)
        {
        nodeCount = 3;
        this.node1Name = node1Name;
        this.node2Name = node2Name;
    ...
    public RoundEllie4MultipleNodes(RoundEllie4Renderer Renderer, String node1Name, String node2Name, String node3Name, String node4Name)
        {
        nodeCount = 4;
        this.node1Name = node1Name;
        this.node2Name = node2Name;
    ...
    public RoundEllie4MultipleNodes(RoundEllie4Renderer Renderer, String node1Name, String node2Name, String node3Name, String node4Name, String node5Name)
        {
        nodeCount = 5;
        this.node1Name = node1Name;
        this.node2Name = node2Name;
    ...
    public RoundEllie4MultipleNodes(RoundEllie4Renderer Renderer, String node1Name, String node2Name, String node3Name, String node4Name, String node5Name, String node6Name)
        {
        nodeCount = 6;
        this.node1Name = node1Name;
        this.node2Name = node2Name;
        ...
        this.node5Name = node5Name;
        this.node5Name = node5Name;
        this.node6Name = node6Name;
        this.renderer = Renderer;

        doTheDisassembly();
        }

    public RoundEllie4MultipleNodes(RoundEllie4Renderer Renderer, String node1Name, String node2Name, String node3Name, String node4Name, String node5Name, String node6Name, String node7Name)
        {
        nodeCount = 7;
        this.node1Name = node1Name;
        this.node2Name = node2Name;
...
// *************************************************************************************************
        ...
        if(nodeCount > 5)
            {
            node6theSpatial = renderer.getNode(node6Name);

            System.out.println("node6theSpatial = " + node6theSpatial);
            al = node6theSpatial.getControllers();

            Tco = node6theSpatial.getController(0);

            Tac = (AnimationController)Tco;
            Tani = Tac.getAnimation(0);
            Tkft = Tani.getKeyFrameTimes();
            if(traceLoad)
                System.out.println("node6Tkft size = " + Tkft.length);
...
// Make storage for the transforms at each keyframe
            ...
            angleZ = new float[Tkft.length];
// Make storage for the Quats that will hold the rotates at each keyframe
            node6keyFrames = new Quaternion[Tkft.length];
// and the vectors that will hold the translates
            node6keyTrans = new Vector3f[Tkft.length];
...
// Get the controller and then cast it to an animation controller
                Controller co = node6theSpatial.getController(k);
                AnimationController ac = (AnimationController)co;
// Get the animations for this controller
                ArrayList bans = ac.getAnimations();
// Step through the animations
...
// Get the frame times for this animation
        ...
        // Build the final key frame Quats from the extracted angles and the Vectors from the extracted translates
            for(int j = 0; j < Tkft.length; j++)
                ...
                if(traceLoad)
                    System.out.println(" ********************** final angles " + angleX[j] + "--" + angleY[j] + "--" + angleZ[j]);
                node6keyFrames[j] = temp;
                node6keyTrans[j] = new Vector3f(transX[j], transY[j], transZ[j]);
                if(traceLoad)
                    System.out.println(" *************** final translations " + transX[j] + " - " + transY[j] + " - " + transZ[j]);
                }
            }
...
// *************************************************************************************************
        ...
        if(nodeCount > 6)
            {
            node7theSpatial = renderer.getNode(node7Name);

            System.out.println("node5theSpatial = " + node5theSpatial);
            al = node7theSpatial.getControllers();

            Tco = node7theSpatial.getController(0);
...
// Make storage for the transforms at each keyframe
            ...
            angleZ = new float[Tkft.length];
// Make storage for the Quats that will hold the rotates at each keyframe
            node7keyFrames = new Quaternion[Tkft.length];
// and the vectors that will hold the translates
            node7keyTrans = new Vector3f[Tkft.length];
...
// Get the controller and then cast it to an animation controller
                Controller co = node7theSpatial.getController(k);
                AnimationController ac = (AnimationController)co;
// Get the animations for this controller
                ArrayList bans = ac.getAnimations();
// Step through the animations
...
// Get the frame times for this animation
        ...
        // Build the final key frame Quats from the extracted angles and the Vectors from the extracted translates
            for(int j = 0; j < Tkft.length; j++)
                ...
                if(traceLoad)
                    System.out.println(" ********************** final angles " + angleX[j] + "--" + angleY[j] + "--" + angleZ[j]);
                node7keyFrames[j] = temp;
                node7keyTrans[j] = new Vector3f(transX[j], transY[j], transZ[j]);
                if(traceLoad)
                    System.out.println(" *************** final translations " + transX[j] + " - " + transY[j] + " - " + transZ[j]);
                }
            }
...
//                    (fromAngles[2] + rotFinalAngles[2] + animationStartRotation[2]));
            ...
            t.addPropertyToInterpolate("Node1Quat", quat, quatNext, new RoundEllie4QuaternionInterpolator());

            ////            t.addPropertyToInterpolate("Node1Quat", node1keyFrames[j], node1keyFrames[j + 1], new RoundEllie4QuaternionInterpolator());
...
////            t.addPropertyToInterpolate("Node1Trans", node1keyTrans[j], node1keyTrans[j + 1], new RoundEllie4VectorInterpolator());
...
// Add each key frame info as an animation and then play the whole
            for(int j = startFrame; j < endFrame; j++)
                {
                t = new Timeline(this);
                t.addPropertyToInterpolate("Node2Quat", node2keyFrames[j], node2keyFrames[j + 1], new RoundEllie4QuaternionInterpolator());
...
///            t.addPropertyToInterpolate("Node2Trans", tempVec, tempVecPlus, new RoundEllie4VectorInterpolator());
///System.out.println("node2 - orig y = " + node2keyTrans[j].y + " - shift - " + tempVec.y);
                t.addPropertyToInterpolate("Node2Trans", node2keyTrans[j], node2keyTrans[j + 1], new RoundEllie4VectorInterpolator());
//        t.setEase(new Spline(0.4f));
                t.setDuration((long)((Tkft[j + 1] - Tkft[j]) * 1000) * animationTimeMultiplier);
...
// Add each key frame info as an animation and then play the whole
            for(int j = startFrame; j < endFrame; j++)
                {
                t = new Timeline(this);
                t.addPropertyToInterpolate("Node3Quat", node3keyFrames[j], node3keyFrames[j + 1], new RoundEllie4QuaternionInterpolator());
...
///            t.addPropertyToInterpolate("Node3Trans", tempVec, tempVecPlus, new RoundEllie4VectorInterpolator());
///System.out.println("node3 - orig y = " + node3keyTrans[j].y + " - shift - " + tempVec.y);
                t.addPropertyToInterpolate("Node3Trans", node3keyTrans[j], node3keyTrans[j + 1], new RoundEllie4VectorInterpolator());
//        t.setEase(new Spline(0.4f));
                t.setDuration((long)((Tkft[j + 1] - Tkft[j]) * 1000) * animationTimeMultiplier);
...
// Add each key frame info as an animation and then play the whole
            for(int j = startFrame; j < endFrame; j++)
                {
                t = new Timeline(this);
                t.addPropertyToInterpolate("Node4Quat", node4keyFrames[j], node4keyFrames[j + 1], new RoundEllie4QuaternionInterpolator());
                t.addPropertyToInterpolate("Node4Trans", node4keyTrans[j], node4keyTrans[j + 1], new RoundEllie4VectorInterpolator());
                t.setDuration((long)((Tkft[j + 1] - Tkft[j]) * 1000) * animationTimeMultiplier);
        ...
        if(nodeCount > 4)
            ...
            for(int j = startFrame; j < endFrame; j++)
                {
                t = new Timeline(this);
                t.addPropertyToInterpolate("Node5Quat", node5keyFrames[j], node5keyFrames[j + 1], new RoundEllie4QuaternionInterpolator());
                t.addPropertyToInterpolate("Node5Trans", node5keyTrans[j], node5keyTrans[j + 1], new RoundEllie4VectorInterpolator());
                t.setDuration((long)((Tkft[j + 1] - Tkft[j]) * 1000) * animationTimeMultiplier);
        ...
        if(nodeCount > 5)
            {
            node6result = new TimelineScenario.Sequence();
            for(int j = startFrame; j < endFrame; j++)
                {
                t = new Timeline(this);
                t.addPropertyToInterpolate("Node6Quat", node6keyFrames[j], node6keyFrames[j + 1], new RoundEllie4QuaternionInterpolator());
                t.addPropertyToInterpolate("Node6Trans", node6keyTrans[j], node6keyTrans[j + 1], new RoundEllie4VectorInterpolator());
                t.setDuration((long)((Tkft[j + 1] - Tkft[j]) * 1000) * animationTimeMultiplier);
                node6result.addScenarioActor(t);
                }
            }

        if(nodeCount > 6)
            {
            node7result = new TimelineScenario.Sequence();
            for(int j = startFrame; j < endFrame; j++)
                {
                t = new Timeline(this);
                t.addPropertyToInterpolate("Node7Quat", node7keyFrames[j], node7keyFrames[j + 1], new RoundEllie4QuaternionInterpolator());
                t.addPropertyToInterpolate("Node7Trans", node7keyTrans[j], node7keyTrans[j + 1], new RoundEllie4VectorInterpolator());
                t.setDuration((long)((Tkft[j + 1] - Tkft[j]) * 1000) * animationTimeMultiplier);
                node5result.addScenarioActor(t);
                }
            }
    ...
    public void setNode6Quat(Quaternion newQuat)
        {
        node6theSpatial.setLocalRotation(newQuat);
        ClientContextJME.getWorldManager().addToUpdateList(node6theSpatial);
        }
    public void setNode6Trans(Vector3f newTrans)
        {
        node6theSpatial.setLocalTranslation(newTrans);
        ClientContextJME.getWorldManager().addToUpdateList(node6theSpatial);
        }

    public void setNode7Quat(Quaternion newQuat)
        {
        node7theSpatial.setLocalRotation(newQuat);
        ClientContextJME.getWorldManager().addToUpdateList(node7theSpatial);
        }
    public void setNode7Trans(Vector3f newTrans)
        {
        node7theSpatial.setLocalTranslation(newTrans);
        ClientContextJME.getWorldManager().addToUpdateList(node7theSpatial);
        }
