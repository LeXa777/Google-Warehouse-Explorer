 ...
 * @author morrisford
 */
public class ModulatorAllNodes {
    ...
    public ModulatorAllNodes(ModulatorRenderer Renderer, ArrayList nodeList)
        {
    ...
    public void allNodesDisassembly()
        {
        Iterator<ModulatorANode> itr = nodeList.iterator();
        while (itr.hasNext())
            {
            ModulatorANode element = itr.next();
            System.out.println("Element = " + element.nodeName);
            doTheDisassembly(element);
            }
        }

    private void doTheDisassembly(ModulatorANode theNode)
        {
